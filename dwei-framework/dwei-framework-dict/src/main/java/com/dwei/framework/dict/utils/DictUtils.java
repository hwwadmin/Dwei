...
public abstract class DictUtils {
    ...
    public static Pair<DictEntity, List<DictDataEntity>> get(String dictCode) {
        var dict = (DictEntity) RedisUtils.support().getOps4hash().get(DictConstant.TYPE_KEY, dictCode);
        if (ObjectUtils.isNull(dict)) return Pair.of();
        //noinspection unchecked
        var dictData = (List<DictDataEntity>) RedisUtils.support().getOps4hash().get(DictConstant.DATA_KEY, dictCode);
    ...
    public synchronized static void refresh(boolean compel) {
        if (!compel && RedisUtils.support().isExist(DictConstant.FLAG_KEY)) return;
        ...
        distributedLock.tryLock(DictConstant.LOCK, 1000, () -> {
            ...
            RedisUtils.support().deleteKeysByPattern(DictConstant.All_PATTER);
            RedisUtils.support().getOps4str().set(DictConstant.FLAG_KEY, true);
            if (ObjectUtils.nonNull(dict))
                RedisUtils.support().getOps4hash().putAll(DictConstant.TYPE_KEY, Maps.pack(Lists.toMap(dict, DictEntity::getCode)));
            if (ObjectUtils.nonNull(dictData))
                RedisUtils.support().getOps4hash().putAll(DictConstant.DATA_KEY, Maps.pack(Lists.toGroup(dictData, DictDataEntity::getDictCode)));
    ...
    public synchronized static void refresh(String dictCode) {
        ...
        distributedLock.tryLock(DictConstant.LOCK, 1000, () -> {
            ...
            RedisUtils.support().getOps4hash().del(DictConstant.TYPE_KEY, dictCode);
            RedisUtils.support().getOps4hash().del(DictConstant.DATA_KEY, dictCode);

            var dict = dictRepository.findByCode(dictCode);
            if (ObjectUtils.isNull(dict)) return null;
            if (!dict.getEnable()) return null;
            RedisUtils.support().getOps4hash().put(DictConstant.TYPE_KEY, dictCode, dict);
            ...
            RedisUtils.support().getOps4hash().put(DictConstant.DATA_KEY, dictCode, dictData);
